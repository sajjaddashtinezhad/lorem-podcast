              ...
              data.allMdx.nodes
                ...
                .map((node) => (
                  <article key={node.id} className="bg-zinc-700 bg-opacity-50 text-white px-8 pt-8 rounded-xl flex flex-col justify-between w-full">
                    ...
                    <Link to={`/episodes/${node.frontmatter.slug}`} className="bg-zinc-900 px-8 py-4 rounded-t-xl block w-min text-nowrap duration-500 hover:px-10" style={{ whiteSpace: 'nowrap' }}>
                      گوش دادن به اپیزود
                    </Link>
                  </article>
                ))
